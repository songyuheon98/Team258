...
function search() {
    ...
    if(keyword != '' & category != '')
        url = `/search/fti?keyword=${keyword}&bookCategoryName=${category}&page=1`
    else if(keyword == '' & category !='')
        url = `/search/fti?bookCategoryName=${category}&page=1`
    else if(keyword != '' & category =='')
        url = `/search/fti?keyword=${keyword}&page=1`
    else if(keyword == '' & category =='')
        url = "/search/fti?page=1"
...
$(document).ready(function() {
    ...
    $('.index-contents').scroll(function() {
        ...
        if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 100 && !loading) {
            ...
            // URL 생성
            let url = '/search/loadMorefti?page=' + currentPage;
